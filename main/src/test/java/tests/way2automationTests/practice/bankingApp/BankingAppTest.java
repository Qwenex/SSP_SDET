...
import org.example.pages.way2automation.practice.bankingApp.BankCustomerLoginPage;
import org.example.pages.way2automation.practice.bankingApp.BankHomePage;
import org.example.pages.way2automation.practice.bankingApp.BankSampleFormPage;
...
public class BankingAppTest extends BaseTest {
    ...
    @BeforeMethod
    public void newPage() {
        bankHomePage = new BankHomePage(webDriver);
        bankHomePage.openPage();
    }
    ...
    public void validRegisterInSampleFormTest() {
        BankSampleFormPage bankSampleFormPage = bankHomePage.moveToSampleFormPage();
        String hobbyWithMaxLength = bankSampleFormPage.getHobbiesList()
                .stream().max(Comparator.comparingInt(String::length)).orElse("");

        String actualMessage = bankSampleFormPage
                .setFirstName("Ivan")
                .setLastName("Ivanov")
    ...
    public void addCustomerInBankManagerLoginTest(String firstName, String lastName, String postCode) {
        String actualMessage = bankHomePage
    ...
    public void openCustomerInBankManagerLoginTest(String firstName, String lastName, String postCode) {
        BankManagerLoginPage bankManagerLoginPage = bankHomePage.moveToBankManagerLoginPage();
    ...
    public void customerTest(String firstName, String lastName, String postCode) {
        ...
        BankCustomerLoginPage bankCustomerLoginPage = bankManagerLoginPage
                .moveToHomePage()
                .moveToCustomerLoginPage();

        String actualMessage = bankCustomerLoginPage
        ...
        String actualMessage1 = bankCustomerLoginPage.enterDeposit(depositAmount);
        ...
        softAssert.assertTrue(bankCustomerLoginPage.isAmountInCreditList(depositAmount),
                "Данные в транзакциях отличаются от ожидаемых");

        // 5.3.2
        Integer zeroDepositAmount = 0;
        String actualMessage2 = bankCustomerLoginPage.enterDeposit(zeroDepositAmount);
        String expectedMessage2 = "";
        softAssert.assertEquals(actualMessage2, expectedMessage2,
                "Сообщение об успешном внесении депозита НЕ должно появится");
        softAssert.assertFalse(bankCustomerLoginPage.isAmountInCreditList(zeroDepositAmount),
                "Данные в транзакциях отличаются от ожидаемых");

        // 5.3.3
        Integer customerBalance = bankCustomerLoginPage.getCustomerBalance();
        Random rnd = new Random();
        Integer rndWithdrawnAmount = rnd.nextInt(customerBalance) + 1;
        String actualMessage3 = bankCustomerLoginPage.takeWithdrawn(rndWithdrawnAmount);
        ...
        softAssert.assertTrue(bankCustomerLoginPage.isAmountInDebitList(rndWithdrawnAmount),
                "Данные в транзакциях отличаются от ожидаемых");

        // 5.3.4
        Integer withdrawnAmount = 1000000;
        String actualMessage4 = bankCustomerLoginPage.takeWithdrawn(withdrawnAmount);
        ...
        softAssert.assertFalse(bankCustomerLoginPage.isAmountInDebitList(withdrawnAmount),
                "Данные в транзакциях отличаются от ожидаемых");

        // 5.3.5
        Integer actualMessage5 = bankCustomerLoginPage.getCustomerBalance();
        Integer expectedMessage5 = bankCustomerLoginPage.getCustomerBalanceFromTransactions();
        softAssert.assertEquals(actualMessage5,expectedMessage5,
                "Баланс, рассчитанный из транзакций отличается от баланса счета");

        // 5.3.6
        Integer customerBalance1 = bankCustomerLoginPage.getCustomerBalance();
        bankCustomerLoginPage.takeWithdrawn(customerBalance1);
        Integer actualMessage6 = bankCustomerLoginPage.getCustomerBalance();
        ...
        boolean actualMessage7 = bankCustomerLoginPage.equalsCountTransaction(expectedTransactions);
        softAssert.assertTrue(actualMessage7,
                "Изначальное количество транзакций должно быть больше 0");

        bankCustomerLoginPage.resetTransactions();

        Integer expectedTransactions1 = 0;
        boolean actualMessage8 = bankCustomerLoginPage.equalsCountTransaction(expectedTransactions1);
        softAssert.assertTrue(actualMessage8,
                "Количество транзакций должно быть равно 0");

        Integer expectedMessage9 = 0;
        Integer actualMessage9 = bankCustomerLoginPage.getCustomerBalance();
