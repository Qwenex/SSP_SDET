...
## Задача T4. Составление тест-плана для тестирования микросервисов.
### Условие задачи: 
* Имеется 2 сервиса: Cервис А и сервис B.
  * Cервис А принимает REST запросы на подключение, отключение и блокирование услуги, отсылает amqp сообщение сервису B.
  * Сервис B выполняет отключение, подключение услуги и сообщает в ответном amqp сообщение результат операции.
...
* Статус услуги изменяется, только после получения положительного ответа от сервиса В.
* Сервис А не отправляет следующую команду, пока не получит ответ на предыдущую команду от сервиса B

![T4Schema.png](screenshots%2Ft4%2FT4Schema.png)

### Требуется:
* Составить план тестирования, <ins>верхнеуровнево</ins> описать проверяемые тест кейсы.
